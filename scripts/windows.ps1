...
function Read-Config {
    ...
    [pscustomobject]@{
        TunnelId=[string]$top.tunnel_id; RuntimeApiKey=[string]$top.runtime_api_key; Port=$port
        RequestedMode=$deployment; DefaultWorkspace=$defaultName; Workspaces=$workspaces
...
function Get-ReleaseAsset([string]$Repo,[string]$Pattern) {
    ...
    if(-not $a){Fail "No matching release asset in ${Repo}: $Pattern"}; return $a
}
function Expand-ZipBinary([string]$Url,[string]$BinaryName,[string]$Destination) {
    ...
    Invoke-WebRequest -Uri $Url -OutFile $archive -UseBasicParsing;Expand-Archive $archive $extract -Force
    $binary=Get-ChildItem $extract -Recurse -File|Where-Object{$_.Name -eq $BinaryName}|Select-Object -First 1
    if(-not $binary){Fail "$BinaryName not found in downloaded archive"};Copy-Item $binary.FullName $Destination -Force
    Remove-Item $archive -Force -ErrorAction SilentlyContinue;Remove-Item $extract -Recurse -Force -ErrorAction SilentlyContinue
}
function Install-TunnelClient { if(-not(Test-Path $TunnelExe)){Fail 'tunnel-client is missing. Run agentdock.cmd again so bootstrap can install it.'} }
function Install-NativeAgentDock([switch]$Force) {
    if((Test-Path $AgentDockExe)-and -not $Force){return};$arch=Get-Arch;$asset=Get-ReleaseAsset 'uvwt/agentdock' "^agentdock_windows_${arch}\.zip$"
    Write-Host 'Installing AgentDock for native mode...';Expand-ZipBinary $asset.browser_download_url 'agentdock.exe' $AgentDockExe;New-Item -ItemType Directory -Force $AgentDockHome|Out-Null
}
function Test-WindowsDocker { if(-not(Get-Command docker -ErrorAction SilentlyContinue)){return $false};& docker info *> $null;if($LASTEXITCODE-ne 0){return $false};& docker compose version *> $null;return($LASTEXITCODE-eq 0) }
function Test-WslDocker { if(-not(Get-Command wsl.exe -ErrorAction SilentlyContinue)){return $false};& wsl.exe -u root -- docker info *> $null;if($LASTEXITCODE-ne 0){return $false};& wsl.exe -u root -- docker compose version *> $null;return($LASTEXITCODE-eq 0) }
function Select-Deployment($Config) {
    if($Config.RequestedMode -eq 'native'){return 'native'}
    if($Config.HasWslWorkspace){if(Test-WslDocker){return 'docker-wsl'};if($Config.RequestedMode-eq'docker'){Fail 'WSL workspace paths require Docker Engine inside WSL.'}}
    else{if(Test-WindowsDocker){return 'docker-windows'};if(Test-WslDocker){return 'docker-wsl'};if($Config.RequestedMode-eq'docker'){Fail 'Docker mode requested but no Docker runtime was found.'}}
    Write-Warning 'Docker unavailable. Native mode has no container directory isolation.';$answer=Read-Host 'Continue with native AgentDock? [y/N]';if($answer-match'^(?i:y|yes)$'){return'native'};Fail 'Install/start Docker Engine and retry.'
}
function New-Token {$b=New-Object byte[] 32;$r=[Security.Cryptography.RandomNumberGenerator]::Create();try{$r.GetBytes($b)}finally{$r.Dispose()};return -join($b|ForEach-Object{$_.ToString('x2')})}
function Get-Token {New-Item -ItemType Directory -Force $Runtime|Out-Null;if(Test-Path $TokenPath){$t=(Get-Content $TokenPath -Raw).Trim();if($t){return$t}};$t=New-Token;Set-Content $TokenPath $t -Encoding ASCII;return$t}
function Get-InstalledMode {if(-not(Test-Path $ModePath)){Fail 'Run install first.'};return(Get-Content $ModePath -Raw).Trim()}
function Convert-ToWslPath([string]$Path) {if(Test-WslPath $Path){return$Path};if($Path-match'^([A-Za-z]):[\\/](.*)$'){$d=$matches[1].ToLowerInvariant();$r=$matches[2].Replace('\','/');return"/mnt/$d/$r"};Fail "Unsupported Windows path for WSL Docker: $Path"}
...
function Write-Compose($Config,[string]$Mode,[string]$Token) {
    ...
    $lines.Add('    environment:');$lines.Add('      AGENTDOCK_HOST: "0.0.0.0"');$lines.Add('      AGENTDOCK_PORT: "8765"');$lines.Add('      AGENTDOCK_OAUTH_ENABLED: "false"');$lines.Add("      AGENTDOCK_AUTH_TOKEN: `"$Token`"");$lines.Add("      AGENTDOCK_DEFAULT_DIR: `"$safeRoot`"")
    $lines.Add('    volumes:');$lines.Add('      - agentdock_home:/home/agentdock/.agentdock')
    # The default workspace is exposed through a stable child path. AgentDock only chmods $safeRoot itself.
...
function Test-PidFile([string]$Path) {if(-not(Test-Path $Path)){return$false};$v=(Get-Content $Path -Raw).Trim();if($v-notmatch'^\d+$'){return$false};try{Get-Process -Id([int]$v)-ErrorAction Stop|Out-Null;return$true}catch{return$false}}
function Start-Native($Config,[string]$Token) {
    Install-NativeAgentDock;$ws=Get-Workspace $Config $Config.DefaultWorkspace;if($ws.PathType-eq'wsl'){Fail 'Native Windows AgentDock cannot use a WSL default workspace.'}
    Write-Warning 'Native mode does not enforce workspace mount isolation.';$env:AGENTDOCK_HOST='127.0.0.1';$env:AGENTDOCK_PORT=[string]$Config.Port;$env:AGENTDOCK_HOME=$AgentDockHome;$env:AGENTDOCK_DEFAULT_DIR=$ws.Path;$env:AGENTDOCK_AUTH_TOKEN=$Token;$env:AGENTDOCK_OAUTH_ENABLED='false'
...
function Start-Tunnel($Config,[string]$Token) {$env:CONTROL_PLANE_API_KEY=$Config.RuntimeApiKey;$env:AGENTDOCK_BEARER_HEADER="Bearer $Token";if(-not(Test-PidFile $TunnelPid)){$p=Start-Process -FilePath $TunnelExe -ArgumentList @('run','--profile-file',$TunnelProfile) -WindowStyle Hidden -PassThru -RedirectStandardOutput $TunnelLog;Set-Content $TunnelPid $p.Id -Encoding ASCII;Start-Sleep 2;if(-not(Test-PidFile $TunnelPid)){Fail 'tunnel-client failed to start. Run logs.'}}}
function Wait-AgentDock([int]$Port) {for($i=0;$i-lt50;$i++){try{$r=Invoke-WebRequest "http://127.0.0.1:$Port/healthz" -UseBasicParsing -TimeoutSec 2;if($r.StatusCode-eq200){return}}catch{};Start-Sleep -Milliseconds 500};Fail 'AgentDock health check failed. Run logs.'}

function Install-Command {$cfg=Read-Config;New-Item -ItemType Directory -Force $Runtime,$Bin|Out-Null;Install-TunnelClient;$mode=Select-Deployment $cfg;$token=Get-Token;Write-TunnelProfile $cfg $token;Set-Content $ModePath $mode -Encoding ASCII;if($mode-like'docker-*'){Write-Compose $cfg $mode $token;Invoke-Compose pull}else{Install-NativeAgentDock};Write-Host "Installed. Default workspace: $($cfg.DefaultWorkspace)" -ForegroundColor Green;Write-Host 'Next: .\agentdock.cmd start'}
function Start-Command {$cfg=Read-Config;Install-TunnelClient;$mode=Get-InstalledMode;$token=Get-Token;Write-TunnelProfile $cfg $token;if($mode-like'docker-*'){Write-Compose $cfg $mode $token;Invoke-Compose up -d --force-recreate}else{Start-Native $cfg $token};Wait-AgentDock $cfg.Port;Start-Tunnel $cfg $token;Write-Host 'AgentDock : RUNNING' -ForegroundColor Green;Write-Host 'Tunnel    : RUNNING' -ForegroundColor Green;Write-Host "Mode      : $mode";Write-Host "Default   : $($cfg.DefaultWorkspace) -> /home/agentdock/AgentDock/default";Write-Host "MCP       : http://127.0.0.1:$($cfg.Port)/mcp"}
function Stop-Command {if(Test-PidFile $TunnelPid){Stop-Process -Id([int](Get-Content $TunnelPid -Raw).Trim()) -Force -ErrorAction SilentlyContinue};Remove-Item $TunnelPid -Force -ErrorAction SilentlyContinue;if(Test-Path $ModePath){$mode=Get-InstalledMode;if($mode-like'docker-*'){if(Test-Path $Compose){Invoke-Compose down}}elseif(Test-PidFile $NativePid){Stop-Process -Id([int](Get-Content $NativePid -Raw).Trim()) -Force -ErrorAction SilentlyContinue}};Remove-Item $NativePid -Force -ErrorAction SilentlyContinue;Write-Host 'Stopped.'}
function Status-Command {$cfg=Read-Config;$a='STOPPED';$t='STOPPED';try{$r=Invoke-WebRequest "http://127.0.0.1:$($cfg.Port)/healthz" -UseBasicParsing -TimeoutSec 2;if($r.StatusCode-eq200){$a='RUNNING'}}catch{};if(Test-PidFile $TunnelPid){$t='RUNNING'};$mode=if(Test-Path $ModePath){Get-InstalledMode}else{'NOT INSTALLED'};Write-Host "AgentDock : $a";Write-Host "Tunnel    : $t";Write-Host "Mode      : $mode";Write-Host "Default   : $($cfg.DefaultWorkspace)";Write-Host "MCP       : http://127.0.0.1:$($cfg.Port)/mcp"}
...
switch($Command){'install'{Install-Command}'start'{Start-Command}'stop'{Stop-Command}'restart'{Apply-Command}'apply'{Apply-Command}'status'{Status-Command}'logs'{Logs-Command}'update'{Update-Command}default{Write-Host 'Usage: .\agentdock.cmd {install|start|stop|restart|apply|status|logs|update}'}}
