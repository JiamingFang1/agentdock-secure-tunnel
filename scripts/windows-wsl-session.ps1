...
function Start-AgentDockWslSession([string]$RuntimeDir,[string]$HelperScript) {
    ...
    try {
        ...
        $shellScript = 'while :; do sleep 3600; done'
        $arguments = @('--distribution',$distro,'--exec','sh','-c',$shellScript,$marker)
        ...
        try {
            ...
            $p.Refresh()
            if ($p.HasExited) {
                throw ("WSL foreground session exited immediately with code {0}." -f $p.ExitCode)
            }
