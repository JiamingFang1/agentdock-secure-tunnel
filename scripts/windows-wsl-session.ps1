...
function Get-WslSessionProcess($Record) {
    ...
    try {
        if ($null -eq $Record -or $Record.Marker -notmatch '^agentdock-keepalive-[0-9a-f]{32} -or [int]$Record.ProcessId -le 0) { return $null }
...
function Start-AgentDockWslSession([string]$RuntimeDir,[string]$HelperScript) {
    ...
    try {
        ...
        $marker = 'agentdock-keepalive-' + [Guid]::NewGuid().ToString('N')

        # Deliberately avoid any project path inside WSL. The sole purpose of
        # this foreground command is to keep the selected distro alive while
        # docker-wsl is serving AgentDock. The marker is argv[0] and therefore
        # remains visible in the Windows wsl.exe command line for PID identity.
        $script = 'while :; do sleep 3600; done'
        $arguments = @('--distribution',$distro,'--exec','sh','-c',$script,$marker)
        ...
        try {
            ...
            $p.Refresh()
            if ($p.HasExited) {
                throw ("WSL foreground session exited immediately (exit {0}). Direct WSL commands may still work; run 'wsl.exe --distribution {1} --exec sh -c ""while :; do sleep 3600; done"" agentdock-test' to diagnose." -f $p.ExitCode,$distro)
            }
...
function Show-AgentDockWslSession([string]$RuntimeDir) {
 ...
 -or [int]$Record.ProcessId -le 0) { return $null }
        $p = Get-Process -Id ([int]$Record.ProcessId) -ErrorAction Stop
        if (-not [string]::Equals($p.Path, $Record.Executable, [StringComparison]::OrdinalIgnoreCase)) { return $null }
        if ($p.StartTime.ToUniversalTime().Ticks.ToString() -ne [string]$Record.StartTicks) { return $null }
        $cim = Get-CimInstance Win32_Process -Filter ("ProcessId={0}" -f [int]$Record.ProcessId) -ErrorAction Stop
